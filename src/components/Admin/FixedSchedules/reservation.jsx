...
const ReserveList = ({
    ...
    const retrieveReservedSubjects = () => {
        ...
        subs?.forEach((subID) => {
            ...
            const count = subjectDays.reduce((acc, _, i) => {
                if (subjectDays[i] === day && subjectPositions[i] === position) {
                    return acc + 1;
                }
                return acc;
            }, 0);
    ...
    const handleAdd = (subID) => {
        const index = days[subID]?.findIndex((d, i) => d === 0 && positions[subID]?.[i] === 0);
        ...
        if (index !== -1) {
            ...
            setPositions((prev) => ({
                ...prev,
                [subID]: prev[subID]?.map((p, i) => (i === index ? position : p)),
            }));
        }
    };

    const handleRemove = (subID) => {
        const index = days[subID]?.findIndex((d, i) => d === day && positions[subID]?.[i] === position);
    ...
    return (
        <div
            key={day === 0 ?`res-g${grade}-p${position}` : `res-g${grade}-d${day}`}
            className='modal modal-open'
        >
            <div className='w-8/12 mx-0 my-0'>
                <div className="flex items-center bg-blue-900 justify-between rounded-t p-4 relative">
                    <h3 className="text-lg text-white font-semibold">
                        {title}
                        {reservationFull && <span className="text-red-500 ml-2">[FULL]</span>}
                    </h3>
                    <button
                        className="btn btn-sm btn-circle btn-ghost text-white absolute top-2 right-2"
                        onClick={onClose}
                    >
                        ✕
                    </button>
                        ...
                        <table className="table table-sm table-zebra w-full">
                            <thead>
                                <tr>
                                    ...
                                    <th className={`w-auto ${editMode ? 'block' : 'hidden'}`}>Actions</th>
                            ...
                            <tbody>
                                {reservedSubjects?.length > 0 ? (
                                    reservedSubjects?.map((subID, index) => {
                                        return (
                                            <tr key={`res_list-g${grade}-s${subID}-d${day}-p${position}`}>
                                                <td>{subjects[subID]?.subject}</td>
                                                <td>{reservedSubjectsCount[subID]}</td>
                                                {editMode &&
                                                    <td>
                                                        <div className=''>
                                                            <button 
                                                                className={`border m-1 ${reservationFull && !checkSubjectList(subID) ? 'bg-gray-400 text-gray-600 cursor-not-allowed' : 'bg-green-500 text-white'}`}
                                                                onClick={() => handleAdd(subID)}
                                                                disabled={reservationFull && !checkSubjectList(subID)}
                                                            >
                                                                <IoMdAdd size={20}/>
                                                            </button>
                                                            <button 
