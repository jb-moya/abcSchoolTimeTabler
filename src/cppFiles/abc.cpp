...
void runExperiment(
	...
	std::unordered_map<int16_t, std::vector<std::pair<int16_t, int16_t>>> section_subjects_units_map = {};
	std::unordered_map<int16_t, std::vector<int16_t>> eligible_teachers_in_subject;
	std::unordered_map<int16_t, int16_t> teachers_total_class_block;
	...
	for (int i = 0; i < teacher_subjects_length; i++) {
		...
		teachers_total_class_block[teacher] = 0;
	}

	for (int i = 0; i < teachers_total_class_block.size(); i++) {
		std::cout << i << " x " << teachers_total_class_block[i] << std::endl;
	}
